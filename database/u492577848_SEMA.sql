...
CREATE TABLE `requerimentos` (
  ...
  `area_total_terreno` varchar(50) DEFAULT NULL COMMENT 'Área total do terreno (desmembramento)',
  `area_remanescente` varchar(50) DEFAULT NULL COMMENT 'Área remanescente após desmembramento',
  `alvara_construcao_numero` varchar(50) DEFAULT NULL COMMENT 'Nº do alvará de construção anterior (habite-se)',
  `eng_fiscal_nome` varchar(255) DEFAULT NULL COMMENT 'Engenheiro fiscal que emitiu o parecer (habite-se)',
  `eng_fiscal_registro` varchar(100) DEFAULT NULL COMMENT 'CREA/registro do engenheiro fiscal (habite-se)',
  `status` enum('Pendente','Em análise','Aguardando Fiscalização','Aprovado','Reprovado','Cancelado','Indeferido','Finalizado','Apto a gerar alvará','Alvará Emitido','Aguardando boleto','Boleto pago','Aguardando Secretaria','Devolvido pela Secretaria','Documento Final Enviado','Aguardando complementação') NOT NULL DEFAULT 'Pendente',
...
ALTER TABLE `assinaturas_digitais`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uq_doc_assinante` (`documento_id`,`assinante_id`),
