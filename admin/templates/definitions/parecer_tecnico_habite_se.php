...
return [
    ...
    'blocos' => [
        ...
        [
            'tipo'   => 'paragrafos',
            'textos' => [
                'Trata o presente parecer técnico sobre o <strong>REQUERIMENTO DE HABITE-SE</strong> referente à edificação localizada em <strong>{{endereco_objetivo}}</strong>, pertencente ao(à) <strong>{{nome_proprietario}}</strong>, CPF/CNPJ <strong>{{cpf_cnpj_proprietario}}</strong>, com responsabilidade técnica conforme <strong>{{responsavel_tecnico_rotulo}} Nº {{responsavel_tecnico_numero}}</strong>, decorrente do Alvará de Construção Nº <strong>{{alvara_construcao_numero}}</strong> e Cadastro Imobiliário Nº <strong>{{cadastro_imobiliario}}</strong>.',
