...
return [
    ...
    'blocos' => [
        ...
        [
            'tipo'   => 'paragrafos',
            'textos' => [
                'Trata o presente parecer técnico sobre o <strong>REQUERIMENTO DE ALVARÁ DE CONSTRUÇÃO</strong>, referente a <strong>{{area_construida}} m²</strong>, no imóvel situado em <strong>{{endereco_objetivo}}</strong>, pertencente ao(à) <strong>{{nome_proprietario}}</strong>, CPF/CNPJ <strong>{{cpf_cnpj_proprietario}}</strong>, com responsabilidade técnica conforme <strong>{{responsavel_tecnico_rotulo}} Nº {{responsavel_tecnico_numero}}</strong>.',
