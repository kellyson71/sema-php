...
// Dados de cada item da fila, disponíveis em JS em qualquer aba — é o que
// permite abrir a modal de assinatura direto do card, sem navegar até a
// página de detalhe primeiro (só clicar no card em si leva pra lá).
$psItensJs = [];
foreach ($filaAssinatura as $it) {
    $psItensJs[$it['documento_id']] = [
        'requerimentoId' => (int) $it['requerimento_id'],
        'protocolo' => $it['protocolo'],
        'tipoDocLegivel' => $it['tipo_doc_legivel'],
        'fiscalNome' => $it['fiscal_nome'],
        'mensagem' => $it['mensagem'],
        'fiscalAssinou' => (bool) $it['fiscal_assinou'],
        'nomeArquivo' => $it['nome_arquivo'],
    ];
}
...
<style>
    ...
    .ps-card-clickable { cursor:pointer; }
    .ps-card-clickable:hover { background:#fbfcfb; }
...
<div class="ps-shell">
        ...
        <section class="ps-panel">
            ...
            <?php if ($filaAssinatura): ?>
                <?php foreach (array_slice($filaAssinatura, 0, 4) as $item): ?>
                    <div class="ps-card ps-card-clickable" onclick="window.location='painel_secretario.php?tab=detalhe&doc=<?= urlencode($item['documento_id']) ?>'">
                        ...
                        <div class="ps-actions">
                            <button type="button" class="ps-btn ps-btn-primary" onclick="event.stopPropagation(); psAbrirModal('<?= htmlspecialchars($item['documento_id'], ENT_QUOTES) ?>')"><i class="fas fa-pen-nib"></i> <?= $item['fiscal_assinou'] ? 'Co-assinar' : 'Assinar' ?></button>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
            ...
            <div style="display:flex;flex-direction:column;gap:14px;">
                <div class="ps-panel">
                    ...
                    <div style="display:flex;align-items:center;gap:14px;padding:15px;">
                        ...
                        <button type="button" class="ps-btn ps-btn-secondary" onclick="psAbrirModal('<?= htmlspecialchars($itemDetalhe['documento_id'], ENT_QUOTES) ?>')"><i class="fas fa-eye"></i> Abrir e assinar</button>
                    </div>
                </div>
            ...
            <div style="display:flex;flex-direction:column;gap:12px;">
                <div class="ps-panel">
                    ...
                    <div style="padding:14px 15px;display:flex;flex-direction:column;gap:8px;">
                        <button type="button" class="ps-btn ps-btn-primary" onclick="psAbrirModal('<?= htmlspecialchars($itemDetalhe['documento_id'], ENT_QUOTES) ?>')"><i class="fas fa-pen-nib"></i> <?= $itemDetalhe['fiscal_assinou'] ? 'Co-assinar' : 'Assinar' ?> e retornar ao setor 2</button>
    ...
    <!-- Modal de assinatura rápida (3 passos) — compartilhada por todas as
         abas: o botão do card abre ela direto (psAbrirModal), clicar no
         card em si é que navega pra página de detalhe. -->
    <div class="ps-modal-backdrop" id="psModalBackdrop">
        <div class="ps-modal">
            <div class="ps-modal-head">
                ...
                <div style="flex:1;">
                    <h3 style="margin:0;font-size:.97rem;font-weight:800;" id="psModalTitulo">Assinar documento</h3>
                    <div style="font-size:.78rem;color:#8fa399;" id="psModalSubtitulo"></div>
                </div>
            ...
            <div class="ps-modal-body">
                ...
                <div id="psPasso1">
                    <div id="psMensagemAlert" class="ps-alert ps-alert-info" style="display:none;"><i class="fas fa-user-pen"></i> <span id="psMensagemTexto"></span></div>

                    <div style="border:1px solid #e3e8e4;border-radius:12px;overflow:hidden;margin-bottom:14px;">
                        <iframe id="psIframe" src="about:blank" style="width:100%;height:260px;border:0;display:block;background:#525659;"></iframe>
                    </div>
                ...
                <div id="psPasso2" style="display:none;">
                    ...
                    <form id="psFormDecisao" method="POST" action="fluxo_setor_handler.php">
                        <input type="hidden" name="requerimento_id" id="psReqIdHidden" value="">
...
const psItens = <?= json_encode($psItensJs) ?>;
let psDocumentoId = null;
let psRequerimentoId = null;
let psMetodoAtual = 'pin';
let psAssinaturaRegistrada = false;

function psAbrirModal(docId) {
    const item = psItens[docId];
    if (!item) { return; }

    psDocumentoId = docId;
    psRequerimentoId = item.requerimentoId;
    psMetodoAtual = 'pin';
    psAssinaturaRegistrada = false;

    document.getElementById('psModalTitulo').textContent = item.fiscalAssinou ? 'Co-assinar documento' : 'Assinar documento';
    document.getElementById('psModalSubtitulo').textContent = item.tipoDocLegivel + ' · ' + item.protocolo;
    document.getElementById('psIframe').src = 'assinatura/redownload_pdf.php?id=' + encodeURIComponent(docId) + '&inline=1';
    document.getElementById('psReqIdHidden').value = item.requerimentoId;

    const mensagemEl = document.getElementById('psMensagemAlert');
    if (item.mensagem) {
        document.getElementById('psMensagemTexto').innerHTML = '<strong>' + item.fiscalNome + '</strong> encaminhou: "' + item.mensagem + '"';
        mensagemEl.style.display = 'flex';
    } else {
        mensagemEl.style.display = 'none';
    }

    // Reseta o estado visual pra quando a mesma modal é reaberta pra outro documento.
    document.getElementById('psPin').value = '';
    document.getElementById('psErro1').style.display = 'none';
    document.getElementById('psRecibo').style.display = 'none';
    document.getElementById('psMotivo').value = '';
    document.getElementById('psMotivoWrap').style.display = 'none';
    psEscolherMetodo('pin');
    psVoltarPasso1();
    psEscolherDecisao('setor3_aprovado', document.querySelector('.ps-decision[data-decisao="setor3_aprovado"]'));

    document.getElementById('psModalBackdrop').classList.add('open');
}
